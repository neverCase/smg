...
async fn v1_embeddings(
    ...
    let pending = state.context.audit_sink.as_ref().and_then(|sink| {
        let raw_request = serde_json::to_vec(&body)
            .map(bytes::Bytes::from)
            .unwrap_or_default();
        sink.prepare_embeddings("/v1/embeddings", &headers, &body, raw_request)
    });
    let response = cancel
        .guard(
            state
...
async fn v1_image_variations(
    ...
    Extension(tenant_meta): Extension<middleware::TenantRequestMeta>,
    cancel: middleware::scheduler::PreemptionGuard,
    ImageVariationMultipart { request, image }: ImageVariationMultipart,
) -> Response {
    if let Err(resp) = check_remote_auth(&state, &headers, &request.model).await {
        return resp;
    }
    cancel
        .guard(state.router.route_image_variations(
            Some(&headers),
            &tenant_meta,
            &request,
            image,
